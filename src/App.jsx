...
const PROC = [
  ...
  { t: "Client Partnership", l: "Adaptive", d: "You work directly with the consultant running your search. Weekly updates, one point of contact, no hand-offs." },
...
function ServiceDetail({ s }) {
  return (
    <div style={{ paddingTop: 6 }}>
      ...
      <Terms items={s.r.split(" · ")} />
    </div>
  );
}
function Services() {
  const w = useWidth(); const mobile = w <= 880;
  const [active, setActive] = React.useState(mobile ? -1 : 0);
  React.useEffect(() => { setActive(mobile ? -1 : 0); }, [mobile]);
  return (
    <section id="services" data-screen-label="Services" style={{ background: "var(--night)", padding: "var(--section-y) 0", position: "relative", overflow: "hidden" }}>
      <div aria-hidden="true" style={{ position: "absolute", width: "min(720px,90vw)", height: "min(720px,90vw)", borderRadius: "50%", background: "var(--glow-red)", filter: "blur(50px)", opacity: .12, pointerEvents: "none", top: `${Math.max(0, active) * 20 - 6}%`, left: "52%", transition: "top 1.2s var(--ease-signature)" }} />
...
function Timeline({ variant }) {
  ...
  const head = (
    <React.Fragment>
      ...
      <Heading style={{ maxWidth: 820, marginBottom: 16 }}>A search unfolds over <em style={{ color: "var(--red)", fontStyle: "italic" }}>120</em> days.</Heading>
      <p style={{ fontSize: "var(--fs-body)", lineHeight: "var(--lh-body)", color: "var(--text-secondary)", maxWidth: 620, marginBottom: "clamp(2.5rem,5vw,4rem)" }}>Every search moves through five phases. Select one to see the work involved and what you receive along the way.</p>
    </React.Fragment>
  );
  if (variant === "b") return (
            ...
            <div>{mobile ? (
              <React.Fragment>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 112px", gap: 16, alignItems: "center", marginBottom: 20 }}>
                  <Heading style={{ fontSize: "clamp(1.75rem,7.5vw,2.3rem)" }}>A search unfolds over <em style={{ color: "var(--red)", fontStyle: "italic" }}>120</em> days.</Heading>
                  <SearchWatch day={watchDay} size={112} />
                </div>
                <p style={{ fontSize: 15, lineHeight: 1.7, color: "var(--text-secondary)", margin: "0 0 28px" }}>Every search moves through five phases. Tap one to see the work involved and what you receive along the way.</p>
              </React.Fragment>
            ) : head}</div>
...
function Bond() {
  ...
  const MEAN = {
    ...
    bsp: "One senior consultant from the first call to the first day. That is the bond.",
  };
  return (
    <section id="bond" ref={ref} data-screen-label="Bond" style={{ padding: "clamp(3rem,6vw,5rem) 0 clamp(3rem,5vw,4.5rem)", background: "var(--night)", textAlign: "center", overflow: "hidden", position: "relative" }}>
          ...
          <div style={{ position: mobile ? "static" : "sticky", top: 110 }}>
            ...
            <div style={{ overflow: "hidden", maxHeight: done ? 400 : 0, opacity: done ? 1 : 0, transition: "all .6s var(--ease-signature)" }}>
              <Card variant="accent" padding={24}>
                ...
                {gaps.length > 0 && <div style={{ marginBottom: 20 }}>{gaps.map((g) => <div key={g.gap} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "7px 0", borderTop: "1px solid var(--white-06)", fontSize: 14, color: "#fff", fontWeight: 600 }}><span style={{ color: "var(--red)", fontSize: 8, marginTop: 6, flexShrink: 0 }}>&#9656;</span>{g.gap}</div>)}</div>}
...
function Chat() {
  ...
  return (
    <React.Fragment>
      <button onClick={() => setOpen(!open)} aria-label={open ? "Close assistant" : "Ask Bound Search Partners"} className="bsp-orb" style={{ position: "fixed", bottom: mobile ? 18 : 26, right: mobile ? 18 : 26, width: 58, height: 58, borderRadius: "50%", padding: 0, background: "radial-gradient(circle at 50% 38%, rgba(48,38,96,.85), rgba(14,11,36,.92) 70%)", border: "1px solid rgba(226,60,65,.35)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", boxShadow: "0 14px 40px rgba(0,0,0,.5), inset 0 1px 0 rgba(255,255,255,.1)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", zIndex: 10001, transition: "transform .4s var(--ease-signature), border-color .4s, box-shadow .4s" }}>
        {!open && <span aria-hidden="true" className="bsp-orb-ring" style={{ position: "absolute", inset: -1, borderRadius: "50%", border: "1px solid rgba(226,60,65,.55)", pointerEvents: "none" }} />}
        {!open && <span aria-hidden="true" className="bsp-orb-ring" style={{ position: "absolute", inset: -1, borderRadius: "50%", border: "1px solid rgba(226,60,65,.35)", pointerEvents: "none", animationDelay: "1.3s" }} />}
        <span style={{ position: "relative", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span className="bsp-orb-core" style={{ position: "absolute", opacity: open ? 0 : 1, transition: "opacity .3s" }} />
          <svg className="bsp-orb-mark" width="16" height="17" viewBox="0 0 130 140" fill="none" aria-hidden="true" style={{ position: "absolute", opacity: 0, transform: "scale(.7)", transition: "all .35s var(--ease-signature)" }}><rect x="4" y="4" width="30" height="132" rx="2" fill="#fff" opacity=".92" /><rect x="56" y="4" width="70" height="60" rx="2" fill="#e23c41" /><rect x="56" y="76" width="70" height="60" rx="2" fill="#e23c41" opacity=".9" /></svg>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" aria-hidden="true" style={{ position: "absolute", opacity: open ? 1 : 0, transform: open ? "rotate(0)" : "rotate(-90deg)", transition: "all .35s var(--ease-signature)" }}><path d="M18 6L6 18M6 6l12 12" /></svg>
        </span>
      </button>
